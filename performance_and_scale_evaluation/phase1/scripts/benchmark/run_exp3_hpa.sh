...
#
# Tests horizontal scaling impact on errors, latency, and RPS.
# Runs multi-turn 50 + single-turn at a fixed concurrency, varying replica count.
# The orchestrator sets replica count externally before each run.
#
# Usage: run_exp3_hpa.sh <replica_count> <concurrency> <turns>
#
# Timing: 30s warmup + 60s measurement = 90s per benchmark
# Default providers: anthropic + vertex (the ones with errors)
...
echo ""
echo "============================================"
echo " EXPERIMENT 3 COMPLETE (replicas=$REPLICA_COUNT)"
